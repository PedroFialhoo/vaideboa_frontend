  ...
  const renderAutocomplete = (
    ...
    <View className="flex-row items-start mb-4 z-50">
      ...
      <View className="flex-1 border-b border-platinum pb-2">
        <Text className="text-gray-400 text-[10px] uppercase font-bold tracking-widest mb-1">{label}</Text>
  ...
  return (
    <View className="flex-1 bg-platinum">
      <View className="bg-velvet-orchid-900 pt-16 pb-12 px-8 rounded-b-[40px] shadow-xl">
        ...
        <Text className="text-purple-x11-200 text-base font-medium">Para onde você quer ir hoje?</Text>
      </View>
      ...
      <View className="px-6 -mt-8">
        <View className="bg-white rounded-3xl p-6 shadow-lg border border-platinum">
          ...
          <View>
            {renderAutocomplete(
              "De onde vamos sair?",
              <View className="bg-purple-x11-100 p-2 rounded-full border-2 border-white shadow-sm"><Navigation size={16} color="#7b4d91" /></View>,
              "Partida de",
              (data, details) => setOrigin({ latitude: details!.geometry.location.lat, longitude: details!.geometry.location.lng, address: data.description }),
              () => setOrigin(null),
              origin?.address
            )}
            ...
            {renderAutocomplete(
              "Para onde vamos?",
              <View className="bg-velvet-orchid-700 p-2 rounded-full border-2 border-white shadow-sm"><MapPin size={16} color="white" /></View>,
              "Destino para",
              (data, details) => setDestination({ latitude: details!.geometry.location.lat, longitude: details!.geometry.location.lng, address: data.description }),
              () => setDestination(null),
              destination?.address
            )}
          </View>
          ...
          <View className="flex-row gap-4 mb-8">
            ...
            >
              ...
              <View className="ml-3">
                ...
                <Text className="text-velvet-orchid-900 font-bold">{date.toLocaleDateString('pt-BR')}</Text>
              </View>
            </TouchableOpacity>
          ...
          {showTimePicker && (
            <DateTimePicker value={date} mode="time" display="default" onChange={onTimeChange} is24Hour={true} />
          )}

          <Pressable 
            disabled={!origin || !destination}
            className={`h-16 rounded-2xl flex-row items-center justify-center shadow-lg active:scale-[0.98] ${origin && destination ? 'bg-velvet-orchid-700' : 'bg-gray-300'}`}
          >
